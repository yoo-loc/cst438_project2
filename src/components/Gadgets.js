...
const Gadgets = () => {
  const [gadgets, gadgetItems] = useState([]); 
  ...
  useEffect(() => {
    fetch('https://wishlistapi-b5777d959cf8.herokuapp.com/items/gadgets')
      ...
      .then(data => {
        gadgetItems(data); // Store fetched gadget items
      })
      .catch(error => {
        console.error('Error fetching gadget items:', error);
        setError('Error fetching gadget items');
      });
  }, []);
  ...
  const handleDeleteItem = (itemId) => {
    ...
    .then(response => {
      if (response.ok) {
        gadgetItems(gadgets.filter(item => item.id !== itemId)); // Remove the deleted item
      } else {
    ...
    <div className="container">
      ...
      {error ? (
        ...
        <div className="gadget-list">
          {gadgets.length > 0 ? (
            gadgets.map(item => (
              <div key={item.id} className="gadget-item card">
                <img 
                src={item.imageURL} 
                alt={item.name} 
